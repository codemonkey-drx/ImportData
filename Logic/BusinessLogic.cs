...
namespace ImportData
{
  public class BusinessLogic
  {
    ...
    public static Sungero.Parties.ICompany GetConterparty(Session session, string companyName, List<Structures.ExceptionsStruct> exceptionList, NLog.Logger logger)
    {
      // TODO Кэшировать.
      ...
      {
        var message = string.Format("Найдено несколько контрагентов с именем \"{0}\". Проверьте, что в выбрана верная запись.", companyName);
        exceptionList.Add(new Structures.ExceptionsStruct {ErrorType = Constants.ErrorTypes.Warn, Message = message});
        logger.Warn(message);
      }
      return company;
